...
つまり「プレーンテキストだから等幅」は、こちらの思い込みでした。空白 4 個ぶんの幅は、`09:00` の後ろでも `(nothing)` の後ろでも同じではありません。**空白で作った列は、受信側で必ず崩れます。**

## 桁揃えをやめて、構造をインデントに移す

最初にやったのは、表をやめることです。

崩れるのは「列」であって「字下げ」ではありません。行頭から何文字ぶん右にあるか、は崩れますが、**「この行は親より内側にある」という関係は残ります**。そこで、時刻と予定名を列で並べるのをやめ、日付ごとのブロックにしました。
